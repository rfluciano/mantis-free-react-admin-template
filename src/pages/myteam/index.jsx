  ...
  export default function MyTeam() {
    ...
    useEffect(() => {
      if (notification?.model === 'Employee') {
        switch (notification.action) {
          ...
          case 'modified':
          case 'deleted':
            fetchEmployees('');  // Call without the searchTerm to see all users
            break;
          default:
    ...
    const fetchEmployees = async (query = '') => {
      setIsLoading(true);
      try {
        const searchQuery = query?.trim?.() || ''; // Safely handle trimming
        let response;
    
        if (searchQuery === '') {
          // Fetch employees managed by the chief
          response = await axis.get(`/employee/chief/${user.matricule}`);
        } else {
          // Fetch employees filtered by search query
          response = await axis.get(`/search/employee`, { params: { query: searchQuery } });
        }
